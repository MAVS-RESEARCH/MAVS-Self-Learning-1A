...
### P10-E004 - Rejected zero-finding run after literal post-freeze contract review

- Date and phase: 2026-07-18 (Asia/Karachi), Phase 10.
- Source checkpoint attempted: `5963890bfdb9c8a3b986ed83d19d7cd2024f0325`; command `node scripts/run_phase10.mjs`; exit `0`; wall time `992.7 s`.
- Successful evidence: focused tests **45/45** before and after; 3,341 frozen upstream artifacts; 40 candidates, 30 semantic classes, 30 behavioral classes, 10 operations, and exact lifecycle reconciliation; 600 certification gates and 6,624 Phase 9 metric comparisons with zero mismatch; 40 permutation challenges invariant; 1,530-artifact taint scan with zero participant influence/retention; 110,445 replay comparisons including all 108,279 protected failures; 6,210,384 trace terminals complete; 174 legacy hashes and all track/bank isolation checks pass; complete regression passes; generated claims are three scoped `supported` and two explicit `unsupported`; final audit reports 25/25 clauses and zero findings; three Ed25519 signatures verify; release freeze reports PASS.
- Post-freeze audit finding: the literal WorkPlan artifact contract was incompletely tested. `REPRODUCE.md`, `manifests/audit_manifest.json`, and `reports/reproducibility_report.md` were absent; the seal did not hash-bind the public key and detached signature records; the candidate input index did not explicitly serialize promotion/rejection/quarantine, rollback, consolidation/retirement, Phase 9 assignment, and descendant-trace links; and clean-environment reproduction had not created a fresh isolated environment from an exact dependency lock.
- Verdict withdrawal: despite numerical PASS and zero reported findings, this run is **rejected and not release-bearing**. Its claims are not accepted. The root seal prevents silent deletion; the complete run will be moved byte-for-byte into `phase10/diagnostic_runs/rejected_5963890_postfreeze_contract_gap/` with an immutable indexed archive before replacement execution.
- Corrective implementation: add the three missing exact artifacts; expand candidate lineage and bank/generation/condition descendant coverage; add `phase10_requirements.lock`; create and delete a fresh isolated virtual environment for the reproduction suite; provide component-level one-command entry points; sign seven manifests/ledgers/audits; bind public key, signatures, release report, result index, and artifact graph into the seal; add read-only full graph verification after freeze; preserve rejected diagnostic evidence outside claim eligibility.
...
### P10-E007 - Rejected R3 attempt after expanded taint and replay gates

- Date and phase: 2026-07-18 (Asia/Karachi), Phase 10.
- Source checkpoint attempted: `3f3e20df25170717a197aee8288d0664ec9c46ec`; command `node scripts/run_phase10.mjs`; exit `1`; wall time `1,658.8 s`.
- Gates completed before rejection: focused tests **53/53**; exact freeze of 3,341 upstream artifacts and 40 candidates; candidate/full-template audit PASS with 30 semantic classes, 30 behavioral classes, 12 structural templates, 20 sibling identities, zero computed constants/no-ops/parent-identical proposals, all 10 name-only variants rejected, all 42 Phase 9 synthesis conditions covered, and exact lifecycle reconciliation; 600 certification gates and 6,624 Phase 9 metrics matched; 40 candidate gate permutations and 414 full Phase 9 metric-label/order challenges were invariant; trace and result-isolation audits passed.
- Failure 1: the expanded taint scan reported five forbidden `unsafe` fields. Two were post-G3 evaluator-side `integrity/retained_counterexample_traces.parquet` artifacts, and three were preserved Phase 1-3 trace schemas outside the Version 0.4 participant surface. The scan correctly failed closed but its zone classifier was overbroad. Correction: explicitly classify retained-counterexample audit traces as evaluator-only and old Phase 1-3 schemas as legacy nonparticipant schemas while continuing to inventory them.
- Failure 2: independent replay reported 16 Phase 7 terminal-authority mismatches. All were valid `ESCALATE` terminals whose authorization field is null by contract and whose residual proof is in `traces/escalations.parquet`; the new checker incorrectly required a local certificate filename for them. Correction: require a matched escalation residual record for `ESCALATE`, and require a local certificate file only for `ACCEPT`/`REJECT`.
- Failure 3: reduced reproduction tests and independent recomputation passed, but its aggregate status was FAIL because it consumes the failed replay status. This is a dependent failure, not a separate implementation defect.
- Replay scale before rejection: 1,752 pinned samples; 2,203,738 protected event records across unsafe acceptance, false rejection, closure error, unexplained escalation, scope leak, taint, gate mismatch, quarantine, rollback, and harmful-promotion categories; 2,100,432 unique selected rows; 33,606,144 component comparisons; 16 authority mismatches and zero reported Phase 9 decision-field mismatch groups.
- Evidence disposition: rejected, unsealed, and not claim-eligible. Step 01 of the replacement run must remove this direct namespace while preserving both immutable rejected sealed-run archives.
- Advancement state: Phase 10 remains **in progress**. A corrective source checkpoint and complete clean restart are required.

### P10-E008 - Accepted authoritative R3 execution and independent release audit

- Date and phase: 2026-07-18 (Asia/Karachi), Phase 10.
- Accepted source checkpoint: `33d806a42bf87c12a62b4d2724274dc957112a80` (`Correct Phase 10 taint and escalation replay audits`). The only pre-run working-tree changes were the permitted Phase 10 result namespace and `results/RESULTS_INDEX.md`; the source/config/test tree was committed. All accepted manifests, claims, signatures, reports, and the seal bind this exact commit.
- Authoritative command: `node scripts/run_phase10.mjs`.
- Exact orchestration result: **PASS**, exit code `0`, wall time `1,496.6 s`; all 19 ordered steps completed through independent frozen-release verification.
- Test gates: Phase 10 focused tests passed **53/53** before evidence generation and **53/53** after the independent audit. The ephemeral clean environment installed the exact `configs/phases/phase10_requirements.lock`, executed **203/203** Phase 6/7/8/10 tests from committed source through pinned `PYTHONPATH`, and was deleted. The complete repository regression passed **411/411** tests. Compile and collection checks also exited `0`.
- Frozen input contract: `input_artifact_index.json` hashes **3,341** Phase 6-9 artifacts and all **40** candidates. Phase binding records contain exact upstream code commits plus hashed config/data/seed/environment/generator identities: Phase 6 `557` data / `4` config / `1` seed / `1` environment / `15` generator artifacts; Phase 7 `465/4/1/1/8`; Phase 8 `702/119/40/1/115`; Phase 9 `1,617/159/2/2/26`. Every per-artifact binding contains its own data SHA-256 and the phase binding-record SHA-256. Input index SHA-256 is `32420abfe752b4a2441fef7d48e1e78c64eb238e1d2baa9e8e08d3c3507480e4`.
- Candidate/full-template evidence: all 40 proposals and the preregistered 30-candidate stratified spot audit reconcile exactly across 10 operations, 30 semantic classes, 30 behavioral classes, 12 structural template signatures, two gate patterns, both rejection reasons, and 20 sibling identities. Computed counts are zero constants, zero no-ops, zero parent-identical candidates, and 10 name-only candidates; all 10 name-only variants are rejected. The lifecycle equation is `40 = 10 integrity_rejected + 10 certification_rejected + 0 quarantined + 20 promoted`. Every promoted candidate has required files, matching hashes, valid operation/witness evidence, both banks, all three generations, all 42 synthesis-condition assignments, later traces, and an exact Phase 9 library match.
- Independent recomputation: **600/600** Phase 6 gate values/vectors and **6,624/6,624** Phase 9 raw-trace metric values match their recorded artifacts within the preregistered `1e-12` tolerances, with zero mismatch.
- Permutation challenge: all 40 candidate gate vectors remain invariant under candidate-name, semantic-neutral ID, operation-label, curriculum-label, generation-label, and candidate-order permutation. All **414** Phase 9 condition-generation metric vectors remain invariant under row-order and condition/generation metadata permutation while physical truth bindings remain fixed. Changed gate/decision/metric outcomes: `0`.
- Hidden-field taint: **3,391** current/legacy/evaluator artifacts and schemas were inventoried. All 13 required surfaces were checked: schemas, world/evaluator outputs, process arguments, environment, open/read files, IPC blind queue, candidate objects, traces, checkpoints, caches, logs, persistent memory, and serialized results. Forbidden participant fields, environment hits, forbidden production imports/paths, and retained sentinels are all `0`. A unique evaluator-only sentinel was passed through the production blind API and rejected; no participant influence or retention occurred.
- Deterministic replay: **1,752** pinned samples plus **2,203,738** protected event records produced **2,100,432** unique selected rows and **33,606,144** independent component comparisons with zero mismatch. Component totals are 2,100,432 terminal, 6,301,296 query/probe/round, 16,802,784 program/scope, and 2,100,432 certificate comparisons. Protected categories are 31,804 unsafe acceptances, 76,475 false rejections, 2,093,807 closure errors in noncertifying controls, 704 scope leaks, 948 gate-mismatch challenge rows, and zero unexplained escalation, taint, harmful promotion, quarantine, or rollback events. The independent engine imports no Phase 9 production executor.
- Trace evidence: **6,210,384** terminal records were checked with completeness rate `1.0`; duplicate/missing terminals, missing hypotheses, missing round lineage, query/probe contract gaps, terminal-authority gaps, lineage/rollback gaps, and residual-decomposition gaps are all zero. Track A contributes 4,635,000 rows/309 condition-generation files/309 checkpoints; Track B contributes 1,575,000/105/105.
- Result isolation: all **174** immutable legacy files match their indexed SHA-256 values; Track A/Track B identity overlap is zero; Phase 6-8 contamination, silent concatenation, and upstream overwrite findings are zero; default reports resolve only to the current Version 0.4 namespace.
- Cumulative-value gate: sealed blind G2/G3 cumulative UAR and FRR are `0/0`, noninferior to fresh, with identical residual escalation floors. Cumulative versus fresh mean rounds are `1.026467 < 1.912667` in G2 and `0.987600 < 1.905867` in G3; latency is `1,239,570 < 1,438,965` and `1,240,140 < 1,446,750`; compute is `46,845 < 60,138` and `46,598 < 60,372`. Scope leakage, anti-scope violations, forgetting, and negative transfer remain zero; library size remains bounded at 20.
- Mechanically generated claims: scoped executable diagnostic synthesis, scoped Perception-Closure runtime, and sealed-bank three-generation blind transfer are `supported`; universal zero error and deployment readiness are `unsupported`. There are zero provisional claims. The claim language is generated from audited gate records, not handwritten elevation.
- Independent final audit: **PASS**, **25/25** WorkPlan clauses true, zero findings. Required scientific/integrity gates pass; `finite_covered_class_only` and `external_operational_validation` remain false by design and force the two unsupported claims rather than blocking the scoped research release.
- Release result: release ID `perception_closure_v04_phase10_20260718_r3`; 45-artifact signed graph; seven valid Ed25519 detached signatures; ephemeral private key not persisted; 37 required contract artifacts present; zero graph mismatches, zero unindexed evidence, and all seven seal checks true. The 57-file authoritative tree occupies 8,116,312 bytes excluding diagnostic archives.
- Exact accepted hashes: audit `5ad16e25fbdf863e511bdccc17ace7616066a71897532cff82829f2675ae1874`; release manifest `a387070f7b85d9fb069857d09fce8a3d9d304822cbb915c974c0edb875dffa78`; claim ledger `0359ad39a339042e7c3ef2ab3353b9d8ab2fc67ca9d8abe95e77b613fa96e44f`; public key `b25eff96bfbc8a5048a5118bf19677adc9ca1da0f7ab57b7d69693f5135ea020`; artifact graph `c148f2305096878ea00a0497c50560cdbbceb1ef35c2265027dc0d387e5aa686`.
- No-training/anti-overfit statement: Phase 10 trains or tunes no model, threshold, diagnostic, or opportunity bank. It reads immutable sealed Phase 6-9 artifacts, freezes the sample before analysis, includes every protected failure/event rather than favorable filtering, independently recomputes results, challenges metadata invariance, and keeps retrospective Track A separate from the sealed blind claim-bearing Track B. Full Phase 9 bank metrics remain authoritative; the reduced replay is only a reproducibility check.
- External publication boundary: `release/tag_record.json` binds the proposed tag to the source commit but records `external_tag_created: false` and `external_tag_authorized: false`, as WorkPlan Section 25 requires separate authorization for an external tag. Repository commit/push is still required under the established phase workflow.

### P10-E009 - Exact Phase 10 console comment and statement registry

Every Phase 10 orchestration step has one factual `console.log` statement with an immediately preceding identifying comment. Focused instrumentation tests require all **19/19** pairs and passed before and after authoritative execution.

| Step | Comment line and exact comment | Statement line and exact statement |
|---|---|---|
| 01 | 11 `// Phase 10 step 01: initialize an isolated unsealed result namespace.` | 12 `console.log('{"event":"phase10.step01.start","action":"isolated_cleanup"}');` |
| 02 | 14 `// Phase 10 step 02: execute focused fail-closed tests before evidence generation.` | 15 `console.log('{"event":"phase10.step02.start","action":"focused_tests_before"}');` |
| 03 | 17 `// Phase 10 step 03: freeze every Phase 6-9 input artifact and candidate linkage.` | 18 `console.log('{"event":"phase10.step03.start","action":"freeze_input_index"}');` |
| 04 | 20 `// Phase 10 step 04: audit candidate-by-candidate lineage and lifecycle reconciliation.` | 21 `console.log('{"event":"phase10.step04.start","action":"candidate_spot_audit"}');` |
| 05 | 23 `// Phase 10 step 05: audit every proposal for semantic and behavioral template integrity.` | 24 `console.log('{"event":"phase10.step05.start","action":"full_template_audit"}');` |
| 06 | 26 `// Phase 10 step 06: independently recompute all certification gates and Phase 9 metrics.` | 27 `console.log('{"event":"phase10.step06.start","action":"independent_recomputation"}');` |
| 07 | 29 `// Phase 10 step 07: challenge name, label, generation, curriculum, and candidate order.` | 30 `console.log('{"event":"phase10.step07.start","action":"permutation_challenge"}');` |
| 08 | 32 `// Phase 10 step 08: audit hidden fields, process access, memory, and sentinel retention.` | 33 `console.log('{"event":"phase10.step08.start","action":"hidden_field_taint"}');` |
| 09 | 35 `// Phase 10 step 09: replay the pinned sample and every protected failure.` | 36 `console.log('{"event":"phase10.step09.start","action":"deterministic_replay"}');` |
| 10 | 38 `// Phase 10 step 10: validate terminal, lineage, authority, influence, and residual traces.` | 39 `console.log('{"event":"phase10.step10.start","action":"trace_completeness"}');` |
| 11 | 41 `// Phase 10 step 11: prove legacy/current and paired/blind result isolation.` | 42 `console.log('{"event":"phase10.step11.start","action":"results_isolation"}');` |
| 12 | 44 `// Phase 10 step 12: execute the independent one-command reduced reproduction package.` | 45 `console.log('{"event":"phase10.step12.start","action":"reproduction_package"}');` |
| 13 | 47 `// Phase 10 step 13: execute the complete repository regression suite.` | 48 `console.log('{"event":"phase10.step13.start","action":"full_regression"}');` |
| 14 | 50 `// Phase 10 step 14: generate claim statuses and maximum language from passed gates.` | 51 `console.log('{"event":"phase10.step14.start","action":"claim_generation"}');` |
| 15 | 53 `// Phase 10 step 15: independently audit every WorkPlan clause and release prerequisite.` | 54 `console.log('{"event":"phase10.step15.start","action":"final_audit"}');` |
| 16 | 56 `// Phase 10 step 16: execute focused tests after final artifact generation.` | 57 `console.log('{"event":"phase10.step16.start","action":"focused_tests_after"}');` |
| 17 | 59 `// Phase 10 step 17: sign manifests, snapshot pointers, and freeze the release namespace.` | 60 `console.log('{"event":"phase10.step17.start","action":"release_freeze"}');` |
| 18 | 62 `// Phase 10 step 18: independently verify graph coverage, seals, claims, and signatures.` | 63 `console.log('{"event":"phase10.step18.start","action":"verify_frozen_release"}');` |
| 19 | 65 `// Phase 10 step 19: report successful completion only after read-only freeze verification.` | 66 `console.log('{"event":"phase10.step19.complete","status":"PASS","release":"FROZEN"}');` |

### P10-E010 - Post-freeze adversarial stress, preservation audit, and final technical verdict

- Seal-enforcement command: `python scripts/clean_phase10_results.py`.
- Exact expected result: exit `1`, `RuntimeError: P10_RELEASE_FROZEN: refusing to clean sealed Phase 10 results`. Audit SHA-256 before/after remained `5AD16E25FBDF863E511BDCCC17ACE7616066A71897532CFF82829F2675AE1874`; seal file SHA-256 before/after remained `FF1556D0B6FE5A771E730413AD5C338CCACECF7D1C8FD54B9D0F3A83215B64DD`.
- Independent frozen verifier rerun: **PASS**; 37 required artifacts, zero missing; seven signatures valid; 45 graph artifacts, zero mismatch; release-manifest, claim-ledger, public-key, release-report, result-index, artifact-graph, and signature seal checks all true; zero unindexed evidence and zero provisional claims.
- Schema stress: accepted audit manifest, claim ledger, release manifest, replay comparison, and result-isolation audit validate against all five Version 0.4 schemas.
- Signature corruption stress: one bit of a copied release-manifest signature was flipped in an OS temporary directory; verification rejected it. The temporary directory was removed. The accepted namespace contains zero private-key files.
- Diagnostic preservation: `rejected_5963890_postfreeze_contract_gap` preserves 49 indexed files / 3,775,099 bytes from source `5963890`; `rejected_e4748db_literal_semantic_audit_gap` preserves 56 indexed files / 5,246,985 bytes from source `e4748db`. Every R2 archive byte/hash was revalidated before the accepted run. Both archives are immutable and non-claim-eligible; neither is in the R3 artifact graph.
- Previous-result preservation: all 174 legacy-index files remain byte-identical; Phases 6-9 input graph SHA-256 remains `1f4b6b5252fa2efdbc9eed9b3d8d2936f4fca6b511d2e40fe0bd154274b49bb4`; no prior result was deleted, overwritten, pooled, or silently replaced.
...
| 10 | Reproducibility, Claim, and Release Audit | Complete | P10-E008 through P10-E010; 3,341-input freeze, 33.61M independent replay comparisons, 411-test regression, 25/25 clauses, seven signatures, frozen graph verification, and post-freeze corruption/immutability stress pass |
