...
def audit_hidden_fields() -> dict[str, Any]:
    ...
    for root in participant_roots:
        for path in sorted(root.rglob("*")):
            ...
            inventory.append({"path": path.relative_to(REPO_ROOT).as_posix(), "field_count": len(keys), "forbidden_fields": hits})
    ...
    for path in sorted((REPO_ROOT / "src" / "mavs10d" / "audit_v04").glob("*.py")):
        ...
        if "mavs10d.certification" in text or "mavs10d.learning" in text or "mavs10d.revalidation" in text:
